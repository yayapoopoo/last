package com.icia.dabyinsa.board.event.service;
...
import com.icia.dabyinsa.board.event.dao.event_BoardDao;
import com.icia.dabyinsa.board.event.dto.event_BfileDto;
import com.icia.dabyinsa.board.event.dto.event_BoardDto;
...
@Service
@Log
public class event_BoardService {
	@Autowired
	private event_BoardDao bDao;
	@Autowired
	...
	public ModelAndView getBoardList(Integer pageNum) {
		...
		List<event_BoardDto> bList = bDao.getList(pmap);
	...
	public String boardInsert(MultipartHttpServletRequest multi,
		...
		event_BoardDto board = new event_BoardDto();
		board.setBid(id);
	...
	public ModelAndView getContents(Integer bnum) {
		...
		event_BoardDto board = bDao.getContents(bnum);
		
		List<event_BfileDto> bfList = bDao.getBfList(bnum);
		
		List<event_ReplyDto> rList = bDao.getReList(bnum);
	...
	@Transactional
	public Map<String, List<event_ReplyDto>> replyInsert(event_ReplyDto reply){
		Map<String, List<event_ReplyDto>> rmap = null;
		
		try {
			bDao.replyInsert(reply);
			
			List<event_ReplyDto> rList = bDao.getReList(reply.getR_bnum());
			
			rmap = new HashMap<String, List<event_ReplyDto>>();
	...
	@Transactional
	public Map<String, List<event_BfileDto>> fileDelete(String sysname, int bnum) {
		...
		try {
			...
			if(file.exists()) {
				if(file.delete()) {
					List<event_BfileDto> fList = bDao.getBfList(bnum);
					rMap = new HashMap<String, List<event_BfileDto>>();
