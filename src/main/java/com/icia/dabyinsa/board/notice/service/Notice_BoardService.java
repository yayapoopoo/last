package com.icia.dabyinsa.board.notice.service;
...
import com.icia.dabyinsa.board.notice.dao.Notice_BoardDao;
...
import com.icia.dabyinsa.board.notice.util.PagingUtil;
...
public class Notice_BoardService {
	...
	public ModelAndView getBoardList(Integer pageNum) {
		...
		List<Notice_BoardDto> bList = bDao.getList(pmap);
		...
		//jsp 파일 이름 지정
		mv.setViewName("board/Notice_boardList");
	...
	public String boardInsert(MultipartHttpServletRequest multi,
		...
		Notice_BoardDto board = new Notice_BoardDto();
		board.setBid(id);
		...
		try {
			...
			view = "redirect:notice_list";//목록의 첫페이지로.
			msg = "글 작성 성공";
		} catch (Exception e) {
			e.printStackTrace();
			view = "redirect:notice_writeFrm";
			msg = "글 작성 실패";
		}
	...
	public ModelAndView getContents(Integer bnum) {
		...
		//글 내용 가져오기
		Notice_BoardDto board = bDao.getContents(bnum);
		//파일 목록 가져오기
		List<Notice_BfileDto> bfList = bDao.getBfList(bnum);
		//댓글 목록 가져오기
		List<Notice_ReplyDto> rList = bDao.getReList(bnum);
		...
		//보여질 페이지(jsp) 이름 지정
		mv.setViewName("board/Notice_boardContents");
	...
	@Transactional
	public Map<String, List<Notice_ReplyDto>> replyInsert(Notice_ReplyDto reply){
		Map<String, List<Notice_ReplyDto>> rmap = null;
		...
		try {
			...
			//댓글 목록 다시 검색
			List<Notice_ReplyDto> rList = bDao.getReList(reply.getR_bnum());
			
			rmap = new HashMap<String, List<Notice_ReplyDto>>();
	...
	public ModelAndView updateFrm(int bnum, 
		...
		mv.setViewName("board/Notice_updateFrm");
	...
	public String boardUpdate(MultipartHttpServletRequest multi,
		...
		log.info("notice_boardUpdate() t: " + title + ", c: " 
				+ contents);
		...
		view = "redirect:notice_contents?bnum=" + bnum;
		
		return view;
	}
	
	@Transactional
	public Map<String, List<Notice_BfileDto>> fileDelete(String sysname, int bnum) {
		...
		try {
			...
			if(file.exists()) {
				if(file.delete()) {
					List<Notice_BfileDto> fList = bDao.getBfList(bnum);
					rMap = new HashMap<String, List<Notice_BfileDto>>();
