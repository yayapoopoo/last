package com.icia.dabyinsa.board.oneone.service;
...
import com.icia.dabyinsa.board.oneone.dao.Oneone_BoardDao;
...
import com.icia.dabyinsa.board.oneone.util.PagingUtil;

import lombok.extern.java.Log;

@Service
@Log
public class Oneone_BoardService {
	@Autowired
	private Oneone_BoardDao bDao;
	@Autowired
	...
	public ModelAndView getBoardList(Integer pageNum, Principal p) {
		...
		List<Oneone_BoardDto> bList = bDao.getList(pmap);
		List<Oneone_BoardDto> bmyList = bDao.getmyList(pmap);

		
		//화면에 전송.
		mv.addObject("bList", bList);
		mv.addObject("bmyList", bmyList);
		...
		//jsp 파일 이름 지정
		mv.setViewName("board/Oneone_boardList");
	...
	public String boardInsert(MultipartHttpServletRequest multi,
		...
		Oneone_BoardDto board = new Oneone_BoardDto();
		board.setBid(id);
		...
		try {
			...
			view = "redirect:oneone_list";//목록의 첫페이지로.
			msg = "글 작성 성공";
		} catch (Exception e) {
			//e.printStackTrace();
			view = "redirect:oneone_writeFrm";
			msg = "글 작성 실패";
		}
	...
	public ModelAndView getContents(Integer bnum) {
		...
		//글 내용 가져오기
		Oneone_BoardDto board = bDao.getContents(bnum);
		//파일 목록 가져오기
		List<Oneone_BfileDto> bfList = bDao.getBfList(bnum);
		//댓글 목록 가져오기
		List<Oneone_ReplyDto> rList = bDao.getReList(bnum);
		...
		//보여질 페이지(jsp) 이름 지정
		mv.setViewName("board/Oneone_boardContents");
	...
	@Transactional
	public Map<String, List<Oneone_ReplyDto>> replyInsert(Oneone_ReplyDto reply){
		Map<String, List<Oneone_ReplyDto>> rmap = null;
		...
		try {
			...
			//댓글 목록 다시 검색
			List<Oneone_ReplyDto> rList = bDao.getReList(reply.getR_bnum());
			
			rmap = new HashMap<String, List<Oneone_ReplyDto>>();
	...
	public ModelAndView updateFrm(int bnum, 
		...
		mv.setViewName("board/Oneone_updateFrm");
	...
	public String boardUpdate(MultipartHttpServletRequest multi,
		...
		log.info("oneone_boardUpdate() t: " + title + ", c: " 
				+ contents);
		...
		view = "redirect:oneone_contents?bnum=" + bnum;
		
		return view;
	}
	
	@Transactional
	public Map<String, List<Oneone_BfileDto>> fileDelete(String sysname, int bnum) {
		...
		try {
			...
			if(file.exists()) {
				if(file.delete()) {
					List<Oneone_BfileDto> fList = bDao.getBfList(bnum);
					rMap = new HashMap<String, List<Oneone_BfileDto>>();
