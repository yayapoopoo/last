package com.icia.dabyinsa.board.infuse.service;
...
import com.icia.dabyinsa.board.infuse.dao.Infuse_BoardDao;
...
import com.icia.dabyinsa.board.infuse.util.PagingUtil;

import lombok.extern.java.Log;

@Service
@Log
public class Infuse_BoardService {
	@Autowired
	private Infuse_BoardDao bDao;
	@Autowired
	...
	public ModelAndView getBoardList(Integer pageNum) {
		...
		List<Infuse_BoardDto> bList = bDao.getList(pmap);
		...
		//jsp 파일 이름 지정
		mv.setViewName("board/Infuse_boardList");
	...
	public String boardInsert(MultipartHttpServletRequest multi,
		...
		Infuse_BoardDto board = new Infuse_BoardDto();
		board.setBid(id);
		...
		try {
			...
			view = "redirect:infuse_list";//목록의 첫페이지로.
			msg = "글 작성 성공";
		} catch (Exception e) {
			//e.printStackTrace();
			view = "redirect:infuse_writeFrm";
			msg = "글 작성 실패";
		}
	...
	public ModelAndView getContents(Integer bnum) {
		...
		//글 내용 가져오기
		Infuse_BoardDto board = bDao.getContents(bnum);
		//파일 목록 가져오기
		List<Infuse_BfileDto> bfList = bDao.getBfList(bnum);
		//댓글 목록 가져오기
		List<Infuse_ReplyDto> rList = bDao.getReList(bnum);
		...
		//보여질 페이지(jsp) 이름 지정
		mv.setViewName("board/Infuse_boardContents");
	...
	@Transactional
	public Map<String, List<Infuse_ReplyDto>> replyInsert(Infuse_ReplyDto reply){
		Map<String, List<Infuse_ReplyDto>> rmap = null;
		...
		try {
			...
			//댓글 목록 다시 검색
			List<Infuse_ReplyDto> rList = bDao.getReList(reply.getR_bnum());
			
			rmap = new HashMap<String, List<Infuse_ReplyDto>>();
	...
	public ModelAndView updateFrm(int bnum, 
		...
		mv.setViewName("board/Infuse_updateFrm");
	...
	public String boardUpdate(MultipartHttpServletRequest multi,
		...
		log.info("infuse_boardUpdate() t: " + title + ", c: " 
				+ contents);
		...
		view = "redirect:infuse_contents?bnum=" + bnum;
		
		return view;
	}
	
	@Transactional
	public Map<String, List<Infuse_BfileDto>> fileDelete(String sysname, int bnum) {
		...
		try {
			...
			if(file.exists()) {
				if(file.delete()) {
					List<Infuse_BfileDto> fList = bDao.getBfList(bnum);
					rMap = new HashMap<String, List<Infuse_BfileDto>>();
